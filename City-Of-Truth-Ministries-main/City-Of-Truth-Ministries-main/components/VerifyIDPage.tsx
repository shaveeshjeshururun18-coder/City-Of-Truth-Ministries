...
const VerifyIDPage = () => {
            ...
            html5Qrcode.start(
                ...
                (decodedText: string) => {
                    const extractId = (text: string) => {
                        // Support both /verify/ and /card/ patterns
                        const patterns = [/\/verify\/([A-Z0-9-]+)/i, /\/card\/([A-Z0-9-]+)/i];
                        for (const pattern of patterns) {
                            const match = text.match(pattern);
                            if (match) return match[1].trim();
                        }
                        // Fallback: take last segment if it looks like a COT ID
                        const lastSegment = text.split('/').pop()?.trim();
                        if (lastSegment?.startsWith('COT-')) return lastSegment;
                        return text.trim();
                    };
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        html5QrCode.scanFile(file, true)
            ...
            .catch((_err: any) => { setError('No valid QR code found in the image.'); setLoading(false); });
    };
