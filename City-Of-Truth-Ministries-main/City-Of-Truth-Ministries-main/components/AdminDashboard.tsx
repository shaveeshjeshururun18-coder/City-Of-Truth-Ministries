...
const HOME_SECTIONS_INFO: Record<string, { name: string; desc: string; icon: any; color: string }> = {
    ...
    about: { name: 'About Ministry', desc: 'Mission, vision and core values', icon: Users, color: 'bg-brand-500' },
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
    ...
    const [adminMenuLayout, setAdminMenuLayout] = useState<'horizontal' | 'vertical'>(() => {
        const saved = localStorage.getItem('adminMenuLayout');
        return (saved === 'horizontal' || saved === 'vertical') ? saved : 'horizontal';
    });

    useEffect(() => {
        localStorage.setItem('adminMenuLayout', adminMenuLayout);
    }, [adminMenuLayout]);

    const [activeTab, setActiveTab] = useState<'users' | 'testimonials' | 'ministries' | 'id-cards' | 'home-layout' | 'menu-editor' | 'pending-approvals'>('users');
    const [testimonials, setTestimonials] = useState<Testimonial[]>([]);
    const [ministries, setMinistries] = useState<Ministry[]>([]);
    const [editingMinistry, setEditingMinistry] = useState<Partial<Ministry> | null>(null);
    const [localNavItems, setLocalNavItems] = useState<any[]>(navItems);
    const [editingNavLabel, setEditingNavLabel] = useState<string | null>(null);
    const [expandedNavItem, setExpandedNavItem] = useState<string | null>(null);
    const [navLabelDraft, setNavLabelDraft] = useState('');
    const [navHasChanges, setNavHasChanges] = useState(false);
    const [hasOrderChanges, setHasOrderChanges] = useState(false);
    const [showAddUserModal, setShowAddUserModal] = useState(false);
    const [addUserForm, setAddUserForm] = useState({ name: '', email: '', phone: '', location: '', role: 'Member' as UserRole, password: '' });
    const [addUserLoading, setAddUserLoading] = useState(false);

    React.useEffect(() => { setLocalNavItems(navItems); }, [navItems]);

    const updateNavItemLabel = (oldLabel: string, newLabel: string) => {
        setLocalNavItems(prev => prev.map(item =>
            item.label === oldLabel ? { ...item, label: newLabel } : item
        ));
        setNavHasChanges(true);
        setEditingNavLabel(null);
    };

    const updateSubLabel = (parentLabel: string, oldSub: string, newSub: string) => {
        setLocalNavItems(prev => prev.map(item =>
            item.label === parentLabel
                ? { ...item, submenu: item.submenu?.map((s: any) => s.label === oldSub ? { ...s, label: newSub } : s) }
                : item
        ));
        setNavHasChanges(true);
    };

    const moveSubItem = (parentLabel: string, idx: number, dir: -1 | 1) => {
        setLocalNavItems(prev => prev.map(item => {
            if (item.label !== parentLabel) return item;
            const subs = [...(item.submenu || [])];
            const target = idx + dir;
            if (target < 0 || target >= subs.length) return item;
        ...
        <div className="min-h-screen bg-slate-50 pt-24 pb-20">
            ...
            <div className="container mx-auto px-6">
                ...
                <div className="flex items-center gap-4 mb-6">
                    <button onClick={onBack} className="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-colors">
                        <ChevronLeft size={20} />
                    </button>
                    <div>
                        <h1 className="text-3xl md:text-4xl font-serif font-bold text-brand-950">Admin Dashboard</h1>
                        <p className="text-slate-500 mt-1 text-sm">Manage users and testimonials</p>
                    </div>
                    <button
                        onClick={() => setAdminMenuLayout(l => l === 'horizontal' ? 'vertical' : 'horizontal')}
                        className="ml-auto flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-xl text-slate-500 hover:text-brand-600 hover:border-brand-300 transition-all text-xs font-bold shadow-sm"
                        title={adminMenuLayout === 'horizontal' ? 'Switch to vertical' : 'Switch to horizontal'}
                    >
                        {adminMenuLayout === 'horizontal' ? (
                            <><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><rect x="3" y="3" width="7" height="18" rx="1" /><rect x="14" y="3" width="7" height="18" rx="1" /></svg> Vertical</>
                        ) : (
                            <><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><rect x="3" y="3" width="18" height="7" rx="1" /><rect x="3" y="14" width="18" height="7" rx="1" /></svg> Horizontal</>
                        )}
                    </button>
                </div>

                {/* ── Layout Wrapper: Horizontal = column, Vertical = row ── */}
                <div className={adminMenuLayout === 'vertical' ? 'flex items-start gap-6' : 'flex flex-col gap-0'}>

                    {/* ── TAB BUTTONS ── */}
                    <div className={adminMenuLayout === 'horizontal'
                        ? 'flex gap-2 flex-wrap mb-6'
                        : 'flex flex-col gap-1 min-w-[180px] bg-white rounded-2xl border border-slate-100 shadow-sm p-3 sticky top-24'
                    }>
                        {([
                            { id: 'users', icon: <Users size={16} />, label: 'Members' },
                            { id: 'testimonials', icon: <MessageSquare size={16} />, label: 'Faith Stories' },
                            { id: 'ministries', icon: <Globe size={16} />, label: 'Global Impact' },
                            { id: 'id-cards', icon: <QrCode size={16} />, label: 'Registry' },
                            { id: 'home-layout', icon: <GripVertical size={16} />, label: 'Page Editor' },
                            { id: 'menu-editor', icon: <Filter size={16} />, label: 'Nav Menu' },
                            { id: 'pending-approvals', icon: <ShieldAlert size={16} />, label: 'Approvals' },
                        ] as { id: typeof activeTab, icon: React.ReactNode, label: string }[]).map(tab => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id)}
                                className={`flex items-center gap-2 font-bold text-sm transition-all ${adminMenuLayout === 'horizontal'
                                        ? `px-4 py-2 rounded-lg ${activeTab === tab.id ? 'bg-brand-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`
                                        : `px-3 py-2.5 rounded-xl text-left w-full ${activeTab === tab.id ? 'bg-brand-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`
                                    }`}
                            >
                                {tab.icon} <span>{tab.label}</span>
                                {tab.id === 'pending-approvals' && users.filter(u => u.photoStatus === 'pending' || u.detailsStatus === 'pending').length > 0 && (
                                    <span className="ml-auto bg-amber-400 text-white text-[9px] px-1.5 py-0.5 rounded-full font-black">
                                        {users.filter(u => u.photoStatus === 'pending' || u.detailsStatus === 'pending').length}
                                    </span>
                                )}
                            </button>
                        ))}
                    ...
                    {/* ── TAB CONTENT ── */}
                    <div className={adminMenuLayout === 'vertical' ? 'flex-1 min-w-0' : 'w-full'}>
                        ...
                        {activeTab === 'users' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                                {[
                                    { label: 'Total Users', value: stats.total, icon: Users, color: 'from-brand-500 to-brand-600', bg: 'bg-brand-50', text: 'text-brand-600' },
                                ...
                                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm mb-6">
                                    <div className="flex flex-col md:flex-row gap-4">
                                        ...
                                        <div className="flex-1 relative flex gap-2">
                                            {/* Add Member button — always visible for admin */}
                                            <button
                                                onClick={() => setShowAddUserModal(true)}
                                                className="px-4 py-3 bg-brand-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-brand-700 transition-colors shadow-sm shrink-0"
                                            >
                                ...
                                {filteredUsers.map((user, index) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex gap-2 pt-4 border-t border-slate-100">
                                            ...
                                            <button
                                                onClick={() => setEditingUser(user)}
                                                className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-brand-50 text-brand-600 rounded-xl font-medium text-sm hover:bg-brand-100 transition-all shadow-sm"
                                            >
                                                <Edit2 size={16} />
                                                Edit
                                            </button>
                                            ...
                                            >
                                                ...
                                                <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-2 z-20">
                                                    ...
                                                    <button
                                                        ...
                                                        className="w-8 h-8 bg-white/90 backdrop-blur shadow-lg rounded-full flex items-center justify-center text-brand-600 hover:bg-brand-600 hover:text-white transition-all transform hover:scale-110"
                                                    >
                                                        <Edit2 size={14} />
                                                    </button>
                                                </div>
                                ...
                                >
                                    <div className="flex items-center justify-between mb-10">
                                        <div>
                                            ...
                                            <p className="text-slate-500 mt-2 text-sm font-medium">Manage and reorder the top navigation links for all visitors.</p>
                                        </div>
                                        <button
                                            onClick={() => {
                                                const label = window.prompt("Enter new menu label:");
                                                if (label && onUpdateNavItems) {
                                                    onUpdateNavItems([...navItems, { label, view: 'HOME' }]);
                                                }
                                            }}
                                            className="flex items-center gap-2 bg-brand-600 text-white px-5 py-2.5 rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-brand-700 transition-all shadow-lg hover:-translate-y-0.5"
                                        >
                                        ...
                                        >
                                            {navItems.map((item) => (
                                                <div key={item.label} className="space-y-2">
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex items-center gap-5 flex-1 relative z-10">
                                                            ...
                                                            <div className="min-w-0">
                                                                ...
                                                                <div className="flex items-center gap-3 mt-0.5">
                                                                    <span className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">{item.view}</span>
                                                                    {item.submenu && item.submenu.length > 0 && (
                                                                        <span className="w-1 h-1 bg-slate-300 rounded-full" />
                                                                    )}
                                                                    {item.submenu && item.submenu.length > 0 && (
                                                                        <button
                                                                            onClick={(e) => { e.stopPropagation(); setExpandedNavItem(expandedNavItem === item.label ? null : item.label); }}
                                                                            className="text-brand-600 text-[10px] font-black uppercase tracking-widest hover:underline"
                                                                        >
                                                                            {item.submenu.length} Sub-links {expandedNavItem === item.label ? '▲' : '▼'}
                                                                        </button>
                                                                    )}
                                                                </div>
                                                            </div>
                                                        ...
                                                        <div className="flex items-center gap-2 relative z-10">
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    const newLabel = window.prompt("Rename menu item:", item.label);
                                                                    if (newLabel && onUpdateNavItems) {
                                                                        onUpdateNavItems(navItems.map(n => n.label === item.label ? { ...n, label: newLabel } : n));
                                                                    }
                                                                }}
                                                                className="w-9 h-9 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-brand-50 hover:text-brand-600 transition-all"
                                                                title="Rename"
                                                            >
                                                                <Edit2 size={16} />
                                                            </button>
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    if (window.confirm(`Delete "${item.label}" menu item?`) && onUpdateNavItems) {
                                                                        onUpdateNavItems(navItems.filter(n => n.label !== item.label));
                                                                    }
                                                                }}
                                                                className="w-9 h-9 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-red-50 hover:text-red-500 transition-all"
                                                                title="Delete"
                                                            >
                                                                <Trash2 size={16} />
                                                            </button>
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    const title = window.prompt("Sub-menu label:");
                                                                    if (title && onUpdateNavItems) {
                                                                        const updated = navItems.map(n => n.label === item.label ? { ...n, submenu: [...(n.submenu || []), { label: title, view: 'HOME' }] } : n);
                                                                        onUpdateNavItems(updated);
                                                                        setExpandedNavItem(item.label);
                                                                    }
                                                                }}
                                                                className="px-3 py-1.5 bg-slate-50 text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-brand-600 hover:text-white transition-all shadow-sm"
                                                            >
                                                                + Sub
                                                            </button>
                                                        </div>
                                                    ...
                                                    {/* Submenu Expansion */}
                                                    {expandedNavItem === item.label && item.submenu && item.submenu.length > 0 && (
                                                        <motion.div
                                                            initial={{ opacity: 0, height: 0 }}
                                                            animate={{ opacity: 1, height: 'auto' }}
                                                            className="ml-14 pl-6 border-l-2 border-slate-100 space-y-2 overflow-hidden"
                                                        >
                                                            <Reorder.Group
                                                                axis="y"
                                                                values={item.submenu}
                                                                onReorder={(newSubOrder) => {
                                                                    if (onUpdateNavItems) {
                                                                        onUpdateNavItems(navItems.map(n => n.label === item.label ? { ...n, submenu: newSubOrder } : n));
                                                                    }
                                                                }}
                                                                className="space-y-2"
                                                            >
                                                                {item.submenu.map((sub: any) => (
                                                                    <Reorder.Item
                                                                        key={sub.label}
                                                                        value={sub}
                                                                        className="bg-slate-50 p-3 rounded-xl border border-slate-200 flex items-center justify-between group/sub cursor-grab active:cursor-grabbing"
                                                                    >
                                                                        <div className="flex items-center gap-3">
                                                                            <GripVertical size={14} className="text-slate-300 group-hover/sub:text-brand-400" />
                                                                            <span className="font-bold text-slate-700 text-xs">{sub.label}</span>
                                                                        </div>
                                                                        <div className="flex items-center gap-1">
                                                                            <button
                                                                                onClick={() => {
                                                                                    const nl = window.prompt("Rename sub-link:", sub.label);
                                                                                    if (nl && onUpdateNavItems) {
                                                                                        onUpdateNavItems(navItems.map(n => n.label === item.label ? { ...n, submenu: n.submenu?.map((s: any) => s.label === sub.label ? { ...s, label: nl } : s) } : n));
                                                                                    }
                                                                                }}
                                                                            ...
                                                                            <button
                                                                                onClick={() => {
                                                                                    if (window.confirm("Delete sub-link?") && onUpdateNavItems) {
                                                                                        onUpdateNavItems(navItems.map(n => n.label === item.label ? { ...n, submenu: n.submenu?.filter((s: any) => s.label !== sub.label) } : n));
                                                                                    }
                                                                                }}
            ...
            {activeTab === 'pending-approvals' && (
                <div className="max-w-5xl mx-auto mt-8">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h2 className="text-3xl font-serif font-black text-brand-950">Pending Approvals</h2>
                            <p className="text-slate-500 mt-2 text-sm font-medium">Review and approve profile edits and photo updates from members.</p>
                        </div>
                    </div>

                    {users.filter(u => u.photoStatus === 'pending' || !!u.pendingEdit || u.status === 'Pending Verification').length > 0 ? (
                        <div className="grid flex-col gap-6">
                            {users.filter(u => u.status === 'Pending Verification').map(user => (
                                <motion.div
                                    key={`register-${user.id}`}
                                    initial={{ opacity: 0, scale: 0.95 }}
                                    animate={{ opacity: 1, scale: 1 }}
                                    className="bg-white p-6 rounded-[2.5rem] border-2 border-brand-500 shadow-xl flex flex-col md:flex-row items-center gap-6"
                                >
                                    <div className="flex items-center gap-4 min-w-[250px]">
                                        <div className="w-16 h-16 bg-brand-50 rounded-full flex items-center justify-center font-bold text-brand-600 text-xl border border-brand-100">
                                            {user.name.charAt(0)}
                                        </div>
                                        <div>
                                            <span className="inline-block px-2 py-0.5 bg-brand-600 text-white text-[10px] font-black uppercase tracking-widest rounded-full mb-1">New Registration</span>
                                            <h3 className="font-bold text-brand-950 text-lg leading-tight">{user.name}</h3>
                                            <p className="text-slate-500 text-xs font-mono">{user.id}</p>
                                        </div>
                                    </div>

                                    <div className="flex-1 text-sm text-slate-600 font-medium">
                                        <div className="grid grid-cols-2 gap-4">
                                            <div><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Email</span> {user.email}</div>
                                            <div><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Phone</span> {user.phone}</div>
                                            <div><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Location</span> {user.location}</div>
                                            <div><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Registered</span> {new Date(user.joinedDate).toLocaleDateString()}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex md:flex-col gap-2 min-w-[160px]">
                                        <button
                                            onClick={() => {
                                                if (window.confirm('Approve this membership?')) {
                                                    onUpdateUser({ ...user, status: 'Active' });
                                                }
                                            }}
                                            className="w-full py-2.5 bg-brand-600 text-white rounded-xl font-bold hover:bg-brand-700 flex items-center justify-center gap-1.5 transition-colors text-sm shadow-md"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="w-full py-2.5 bg-slate-100 text-slate-600 rounded-xl font-bold hover:bg-red-50 hover:text-red-600 transition-colors text-sm"
                                        >
                            ...
                            {users.filter(u => u.photoStatus === 'pending').map(user => (
                                <motion.div
                                    key={`photo-${user.id}`}
                                    initial={{ opacity: 0, y: 10 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    className="bg-white p-6 rounded-[2rem] border border-brand-100 shadow-md flex flex-col md:flex-row gap-6 items-center justify-between"
                                >
                                    <div className="flex items-center gap-4 min-w-[250px]">
                                        <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center font-bold text-slate-400 text-xl border border-slate-200">
                                            {user.name.charAt(0)}
                                        </div>
                                        <div>
                                            <span className="inline-block px-2 py-0.5 bg-amber-100 text-amber-700 text-[10px] font-black uppercase tracking-widest rounded-full mb-1">Photo Update</span>
                                            <h3 className="font-bold text-brand-950 text-lg leading-tight">{user.name}</h3>
                                            <p className="text-slate-500 text-xs font-mono">{user.id}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex justify-center items-center gap-4 flex-1">
                                        <div className="text-center">
                                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Old Photo</p>
                                            <div className="w-24 h-24 rounded-2xl overflow-hidden border border-slate-100 shadow-sm mx-auto">
                                                <img src={user.photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=1e1b4b&color=fff&bold=true&size=256`} className="w-full h-full object-cover opacity-50 grayscale" alt="Current" />
                                            </div>
                                        </div>
                                        <ChevronRight size={24} className="text-slate-300" />
                                        <div className="text-center">
                                            <p className="text-[9px] font-bold text-brand-600 uppercase tracking-widest mb-2">New Upload</p>
                                            <div className="w-24 h-24 rounded-2xl overflow-hidden border-2 border-brand-400 shadow-lg mx-auto">
                                                <img src={user.pendingPhoto || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=1e1b4b&color=fff&bold=true&size=256`} className="w-full h-full object-cover" alt="Pending" />
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex md:flex-col gap-2 min-w-[140px]">
                                        <button
                                            onClick={() => {
                                                if (window.confirm('Approve this photo?')) {
                                                    onUpdateUser({ ...user, photo: user.pendingPhoto, pendingPhoto: undefined, photoStatus: undefined });
                                                }
                                            }}
                                            className="w-full py-2.5 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 flex items-center justify-center gap-1.5 transition-colors text-sm shadow-sm"
                                        >
                                            <CheckCircle size={16} /> Approve
                                        </button>
                                        <button
                                            onClick={() => {
                                                if (window.confirm('Reject this photo update?')) {
                                                    onUpdateUser({ ...user, pendingPhoto: undefined, photoStatus: undefined });
                                                }
                                            }}
                                            className="w-full py-2.5 bg-red-50 text-red-600 rounded-xl font-bold hover:bg-red-100 flex items-center justify-center gap-1.5 transition-colors text-sm"
                                        >
                            ...
                            {users.filter(u => !!u.pendingEdit).map(user => (
                                <motion.div
                                    key={`details-${user.id}`}
                                    initial={{ opacity: 0, y: 10 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    className="bg-white p-6 rounded-[2rem] border border-brand-100 shadow-md flex flex-col gap-6"
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <span className="inline-block px-2 py-0.5 bg-indigo-100 text-indigo-700 text-[10px] font-black uppercase tracking-widest rounded-full mb-1">Profile Details Update</span>
                                            <h3 className="font-bold text-brand-950 text-lg leading-tight">{user.name}</h3>
                                            <p className="text-slate-500 text-xs font-mono">{user.id}</p>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 bg-slate-50 p-6 rounded-[2rem] gap-8 overflow-hidden border border-slate-100">
                                        <div>
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Current Profile Information</p>
                                            <div className="space-y-3">
                                                <div className="text-sm flex items-start"><span className="text-slate-400 font-bold uppercase text-[9px] tracking-tight w-20 shrink-0 mt-1">Name:</span> <span className="text-slate-700 font-medium">{user.name || '—'}</span></div>
                                                <div className="text-sm flex items-start"><span className="text-slate-400 font-bold uppercase text-[9px] tracking-tight w-20 shrink-0 mt-1">Phone:</span> <span className="text-slate-700 font-medium">{user.phone || '—'}</span></div>
                                                <div className="text-sm flex items-start"><span className="text-slate-400 font-bold uppercase text-[9px] tracking-tight w-20 shrink-0 mt-1">Email:</span> <span className="text-slate-700 font-medium">{user.email || '—'}</span></div>
                                                <div className="text-sm flex items-start"><span className="text-slate-400 font-bold uppercase text-[9px] tracking-tight w-20 shrink-0 mt-1">Location:</span> <span className="text-slate-700 font-medium">{user.location || '—'}</span></div>
                                                <div className="text-sm flex items-start"><span className="text-slate-400 font-bold uppercase text-[9px] tracking-tight w-20 shrink-0 mt-1">Emergency:</span> <span className="text-slate-700 font-medium">{user.emergency || '—'}</span></div>
                                            </div>

                                        </div>
                                        <div className="relative">
                                            <div className="absolute -left-4 inset-y-0 w-px bg-slate-200 hidden md:block"></div>
                                            <p className="text-[10px] font-black text-brand-600 uppercase tracking-[0.2em] mb-4">Requested Profile Changes</p>
                                            <div className="space-y-3">
                                                <div className="text-sm flex items-start">
                                                    <span className="text-slate-400 font-bold uppercase text-[9px] tracking-tight w-20 shrink-0 mt-1">Name:</span>
                                                    <span className={user.name !== user.pendingEdit?.name ? 'text-brand-700 font-bold bg-brand-50 px-2 py-0.5 rounded-lg border border-brand-100 shadow-sm' : 'text-slate-700 font-medium italic opacity-60'}>
                                                        {user.pendingEdit?.name || '—'}
                                                    </span>
                                                </div>
                                                <div className="text-sm flex items-start">
                                                    <span className="text-slate-400 font-bold uppercase text-[9px] tracking-tight w-20 shrink-0 mt-1">Phone:</span>
                                                    <span className={user.phone !== user.pendingEdit?.phone ? 'text-brand-700 font-bold bg-brand-50 px-2 py-0.5 rounded-lg border border-brand-100 shadow-sm' : 'text-slate-700 font-medium italic opacity-60'}>
                                                        {user.pendingEdit?.phone || '—'}
                                                    </span>
                                                </div>
                                                <div className="text-sm flex items-start">
                                                    <span className="text-slate-400 font-bold uppercase text-[9px] tracking-tight w-20 shrink-0 mt-1">Email:</span>
                                                    <span className={user.email !== user.pendingEdit?.email ? 'text-brand-700 font-bold bg-brand-50 px-2 py-0.5 rounded-lg border border-brand-100 shadow-sm' : 'text-slate-700 font-medium italic opacity-60'}>
                                                        {user.pendingEdit?.email || '—'}
                                                    </span>
                                                </div>
                                                <div className="text-sm flex items-start">
                                                    <span className="text-slate-400 font-bold uppercase text-[9px] tracking-tight w-20 shrink-0 mt-1">Location:</span>
                                                    <span className={user.location !== user.pendingEdit?.location ? 'text-brand-700 font-bold bg-brand-50 px-2 py-0.5 rounded-lg border border-brand-100 shadow-sm' : 'text-slate-700 font-medium italic opacity-60'}>
                                                        {user.pendingEdit?.location || '—'}
                                                    </span>
                                                </div>
                                                <div className="text-sm flex items-start">
                                                    <span className="text-slate-400 font-bold uppercase text-[9px] tracking-tight w-20 shrink-0 mt-1">Emergency:</span>
                                                    <span className={user.emergency !== user.pendingEdit?.emergency ? 'text-brand-700 font-bold bg-brand-50 px-2 py-0.5 rounded-lg border border-brand-100 shadow-sm' : 'text-slate-700 font-medium italic opacity-60'}>
                                                        {user.pendingEdit?.emergency || '—'}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <button
                                            onClick={() => {
                                                if (window.confirm('Reject these changes?')) {
                                                    onUpdateUser({ ...user, pendingEdit: undefined });
                                                }
                                            }}
                                            className="px-6 py-2.5 bg-red-50 text-red-600 rounded-xl font-bold hover:bg-red-100 transition-colors text-sm"
                                        >
                                            Reject Edit
                                        </button>
                                        <button
                                            onClick={() => {
                                                if (window.confirm('Approve these changes?')) {
                                                    onUpdateUser({ ...user, ...user.pendingEdit, pendingEdit: undefined } as User);
                                                }
                                            }}
                                            className="px-6 py-2.5 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 transition-colors text-sm shadow-sm flex items-center gap-2"
                                        >
                                            <CheckCircle size={16} /> Approve Edit
                                        </button>
                                    </div>
                                </motion.div>
                            ))}
                        ...
                        <div className="text-center py-20 bg-white rounded-[3rem] border border-slate-100 shadow-sm">
                            <ShieldAlert size={64} className="mx-auto mb-6 text-slate-200" />
                            <h3 className="text-2xl font-serif font-black text-brand-950 mb-2">No Pending Approvals</h3>
                            <p className="text-slate-500 font-medium">All member updates have been reviewed.</p>
                        </div>
                    )}
                </div>
            )}
            ...
            {/* ── ADD MEMBER MODAL (admin, no approval needed) ── */}
            <AnimatePresence>
                {showAddUserModal && (
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        ...
                        >
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h2 className="text-2xl font-black text-brand-950 font-serif">Add New Member</h2>
                                    <p className="text-slate-500 text-sm mt-1">Member will be added as <span className="font-bold text-green-600">Active</span> immediately.</p>
                                </div>
                                <button onClick={() => setShowAddUserModal(false)} className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-200 transition-colors">
                                    <X size={18} />
                                </button>
                            </div>
                            ...
                            <div className="space-y-4">
                                {[
                                    { field: 'name', label: 'Full Name *', placeholder: 'e.g. אֲבִיגַיִל Sister', type: 'text' },
                                    { field: 'phone', label: 'Phone / Emergency *', placeholder: '+91 98765 43210', type: 'tel' },
                                    { field: 'email', label: 'Email', placeholder: 'member@example.com', type: 'email' },
                                    { field: 'location', label: 'Location', placeholder: 'City, Country', type: 'text' },
                                    { field: 'password', label: 'Login Password', placeholder: 'Leave blank for default: password', type: 'text' },
                                ].map(({ field, label, placeholder, type }) => (
                                    <div key={field}>
                                        <label className="text-xs font-black text-slate-500 uppercase tracking-widest mb-1.5 block">{label}</label>
                                        <input
                                            type={type}
                                            value={(addUserForm as any)[field]}
                                            onChange={e => setAddUserForm(f => ({ ...f, [field]: e.target.value }))}
                                            placeholder={placeholder}
                                            className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100 transition-all text-sm"
                                        />
                                    </div>
                                ))}
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-brand-500 transition-all text-sm font-bold"
                                    >
                                        {['Member', 'Ministry Leader', 'Choir', 'Media Team', 'Admin'].map(r => <option key={r} value={r}>{r}</option>)}
                                    </select>
                                </div>
                            </div>

                            <div className="flex gap-3 mt-7">
                                <button onClick={() => setShowAddUserModal(false)} className="flex-1 py-3 bg-slate-100 text-slate-600 rounded-xl font-bold text-sm hover:bg-slate-200 transition-colors">Cancel</button>
                                <button
                                    disabled={addUserLoading || !addUserForm.name || !addUserForm.phone}
                                    onClick={async () => {
                                        if (!addUserForm.name || !addUserForm.phone) return;
                                        setAddUserLoading(true);
                                        try {
                                            const id = `COT-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).slice(2, 5).toUpperCase()}`;
                                            const newUser: User = {
                                                id,
                                                name: addUserForm.name.trim(),
                                                email: addUserForm.email.trim(),
                                                phone: addUserForm.phone.trim(),
                                                emergency: addUserForm.phone.trim(),
                                                password: addUserForm.password || 'password',
                                            ...
                                            setAddUserForm({ name: '', email: '', phone: '', location: '', role: 'Member', password: '' });
                                            alert(`✅ Member "${newUser.name}" added! ID: ${newUser.id}`);
                                        } catch (e) {
                                    ...
                                    className="flex-1 py-3 bg-brand-600 text-white rounded-xl font-bold text-sm hover:bg-brand-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                                >
                                    {addUserLoading ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Adding...</> : <><CheckCircle size={16} /> Add as Active Member</>}
                                </button>
                            </div>
                        </motion.div>
                    </div>
            ...
            <AnimatePresence>
                {viewingQrUser && (
                    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        ...
                        >
                            {/* ── Premium QR Code Modal using react-qr-code ── */}
                            <div className="flex justify-end mb-2">
                                <button onClick={() => setViewingQrUser(null)} className="text-slate-400 hover:text-slate-600"><X size={24} /></button>
                            </div>

                            <p className="text-[9px] font-black text-brand-500 uppercase tracking-[3px] text-center mb-1">Member Verification</p>
                            <h3 className="text-xl font-black text-brand-950 text-center mb-0.5">{viewingQrUser.name}</h3>
                            <p className="text-slate-400 font-mono text-xs text-center mb-5">{viewingQrUser.id}</p>

                            {/* QR Code — encodes the /verify/{id} URL */}
                            <div className="bg-gradient-to-br from-slate-50 to-brand-50 p-5 rounded-2xl border border-slate-100 mb-5 flex flex-col items-center gap-3">
                                <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-100">
                                    <QRCode
                                        id="admin-qr-svg"
                                        value={`${window.location.origin}/verify/${viewingQrUser.id}`}
                                        size={180}
                                        fgColor="#1e1b4b"
                                        bgColor="#ffffff"
                                        level="H"
                                ...
                                <p className="text-[9px] text-slate-400 font-medium text-center">
                                    {window.location.origin}/verify/{viewingQrUser.id}
                                </p>
                            </div>
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        if (svg) {
                                            ...
                                            const a = document.createElement('a'); a.href = url; a.download = `QR-${viewingQrUser?.id}.svg`; a.click();
                                            URL.revokeObjectURL(url);
                                        }
                                    }}
                                    className="flex-1 py-3 bg-brand-600 text-white rounded-xl font-bold text-sm hover:bg-brand-700 transition-colors flex items-center justify-center gap-2"
                                >
                    ...
                    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <div className="mt-8 pt-6 border-t border-slate-200">
                                <h4 className="text-sm font-bold text-brand-950 mb-4 uppercase tracking-wider">Community Profile Data</h4>
                                {viewingDetailsUser.communityProfile ? (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-brand-50/50 p-6 rounded-2xl border border-brand-100/50">
                                        {[
                                            { label: 'Denomination', value: viewingDetailsUser.communityProfile.denomination },
                                            { label: 'Church Name', value: viewingDetailsUser.communityProfile.churchName },
                                            { label: 'Role in Ministry', value: viewingDetailsUser.communityProfile.role },
                                            { label: 'Bio / Testimony', value: viewingDetailsUser.communityProfile.bio }
                                        ].map(field => field.value && (
                                            <div key={field.label} className={`break-words ${field.label === 'Bio / Testimony' ? 'md:col-span-2' : ''}`}>
                                                <div className="text-[10px] font-black uppercase text-slate-500 tracking-widest">{field.label}</div>
                                                <div className="text-sm font-semibold text-brand-950 mt-0.5 whitespace-pre-wrap">{field.value}</div>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
                                    <div className="text-sm text-slate-500 italic bg-slate-50 p-6 rounded-2xl border-2 border-dashed border-slate-200">No community profile submitted yet.</div>
                                )}
                            </div>
                            ...
                            <div className="mt-8 pt-6 border-t border-slate-200">
                                ...
                                <div className="flex flex-col md:flex-row items-center gap-6 bg-slate-50 p-6 rounded-3xl border border-slate-100">
                                    <div id="admin-details-qr-container" className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
                                        <QRCode
                                            value={`${window.location.origin}/verify/${viewingDetailsUser.id}`}
                                            size={120}
                                            level="H"
                                    ...
                                    <div className="flex-1 text-center md:text-left">
                                        ...
                                        <button
                                            onClick={async () => {
                                                const node = document.getElementById('admin-details-qr-container');
                                                if (node) {
                                                    try {
                                                        const dataUrl = await toPng(node, { pixelRatio: 3, backgroundColor: '#ffffff', cacheBust: true });
                                                        const a = document.createElement('a');
                                                        a.href = dataUrl;
                                                        a.download = `QR-${viewingDetailsUser.id}.png`;
                                                        a.click();
                                                    } catch (err) {
                                                        alert("Failed to download QR. Try again.");
                                                    }
                                                }
                                            }}
