...
export const AuthPage: React.FC<AuthPageProps> = ({
    ...
    const extractQrId = (raw: string) => {
        ...
        if (text.includes('/verify/')) {
            const part = text.split('/verify/')[1]?.split('?')[0]?.trim();
            if (part) return part;
        }

        if (text.includes('/card/')) {
            const part = text.split('/card/')[1]?.split('?')[0]?.trim();
            if (part) return part;
        }
    ...
    const ensureHtml5Qrcode = () => new Promise<void>((resolve, reject) => {
        ...
        script.async = true;
        script.setAttribute('data-html5qrcode', 'true');
        script.onload = () => resolve();
        script.onerror = () => reject(new Error('QR script failed'));
    ...
    const startScanner = async () => {
        ...
        try {
            ...
            const html5 = new (window as any).Html5Qrcode('qr-auth-page-camera');
            scannerRef.current = html5;
            await html5.start(
                { facingMode: 'environment' },
                { fps: 10, qrbox: { width: 260, height: 260 } },
                (decodedText: string) => {
                    const extracted = extractQrId(decodedText);
                    if (!extracted) return;
            ...
            setScannerError('Camera access blocked or not available. Use Upload instead.');
            setScannerBusy(false);
        }
    };
    ...
    const handleFileQRScan = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const doScan = () => {
            ...
            h5.scanFile(file, true)
                .then((text: string) => {
                    const qrData = extractQrId(text) || text.trim();
        ...
        ensureHtml5Qrcode()
            ...
            .catch(() => {
                setScanningFile(false);
                alert('QR scanner failed to load. Please try again.');
            });
    };
    ...
    const handleProceed = () => {
        if (previewUser) {
            onLogin(identifier || previewUser.id);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col relative text-brand-900">
            ...
            <div className="h-64 md:h-80 bg-gradient-to-br from-brand-900 to-brand-950 relative flex flex-col items-center justify-center overflow-hidden flex-shrink-0 px-6">
                ...
                <button
                    onClick={onBack}
                    className="absolute top-8 left-6 md:left-12 flex items-center gap-2 text-white/90 hover:text-white transition-all bg-white/10 hover:bg-white/20 px-5 py-2.5 rounded-full border border-white/20 group active:scale-95 shadow-lg"
                >
                    ...
                    {view === 'login' && (
                        ...
                        >
                            <div className="bg-white/80 p-10 md:p-16 rounded-[4rem] border border-brand-50 shadow-[0_30px_100_rgba(59,130,246,0.05)] text-center relative overflow-hidden">
                                ...
                                <p className="text-xs text-brand-300 font-black uppercase tracking-[0.2em]">
                                    Search by COT ID, name, phone number, or email address.
                                </p>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-12">
                                    ...
                                    >
                                        ...
                                        <h4 className="font-black text-xl mb-2 tracking-tight">{showScanner ? 'Close Scanner' : 'Use QR Scanner'}</h4>
                            ...
                            {/* QR Scanner Panel */}
                            {showScanner && (
                                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="overflow-hidden rounded-[3rem] border-4 border-brand-500/20 bg-slate-950 shadow-2xl">
                                    <div className="p-4 bg-slate-900 text-white text-xs text-center font-black uppercase tracking-[0.3em] flex justify-between items-center px-8 border-b border-white/5">
                                    ...
                                    <div className="p-4 relative">
                                        <div className="w-full h-[420px] rounded-[2.5rem] overflow-hidden border border-white/10 bg-slate-900">
                                            <div id="qr-auth-page-camera" className="w-full h-full" />
                                        </div>
                                        {scannerBusy && !scannerError && (
                                            <div className="mt-4 text-center text-white/60 text-xs font-black uppercase tracking-[0.3em]">
                                                Starting camera...
                                            </div>
                                        )}
                                        {scannerError && (
                                            <div className="mt-4 text-center text-red-200 text-xs font-black uppercase tracking-[0.2em]">
                                                {scannerError}
                                            </div>
                                        )}
            ...
            <footer className="py-12 border-t border-brand-50 relative z-10 bg-brand-50">
