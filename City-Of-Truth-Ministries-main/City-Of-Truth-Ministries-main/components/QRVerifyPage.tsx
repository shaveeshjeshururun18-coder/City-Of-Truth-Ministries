...
export const QRVerifyPage: React.FC<QRVerifyPageProps> = ({ userId, onBack, onScan, users = [] }) => {
            ...
            <AnimatePresence mode="wait">
                ...
                {!loading && error && (
                    <motion.div key="error" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md bg-white rounded-3xl p-8 shadow-xl border border-red-100 text-center">
                        ...
                        <div className="mt-6 p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6">
                        ...
                        {onScan && (
                            <button 
                                onClick={onScan}
                                className="w-full flex items-center justify-center gap-2 py-4 px-6 bg-brand-600 text-white font-bold rounded-2xl shadow-lg hover:shadow-xl transition-all"
                            >
                                <Shield size={18} /> Try Searching or Scanning Again
                            </button>
                        )}
                    </motion.div>
                )}
