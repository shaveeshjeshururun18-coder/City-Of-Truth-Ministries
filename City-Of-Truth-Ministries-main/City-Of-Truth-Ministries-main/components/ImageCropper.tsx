...
import { X, Check, ZoomIn, Move, UploadCloud } from 'lucide-react';
...
interface ImageCropperProps {
    ...
    onCancel: () => void;
    onNewPhoto?: (e: React.ChangeEvent<HTMLInputElement>) => void;
    isNewUpload?: boolean;
}

export const ImageCropper: React.FC<ImageCropperProps> = ({ imageSrc, onCropComplete, onCancel, onNewPhoto, isNewUpload }) => {
    ...
    useEffect(() => {
        ...
        img.onload = () => {
            if (canvasRef.current) {
                ...
                const newScale = Math.min(scaleX, scaleY) * 0.9; // 90% of view for breathing room
                setScale(newScale);
    ...
    const handleCrop = () => {
        ...
        if (img.complete && img.naturalWidth > 0) {
            ...
            // Target a high-quality resolution (e.g., 512x512 max or keep source size if smaller)
            const targetSize = 800;
            const scaleFactor = Math.min(1, targetSize / Math.max(sourceWidth, sourceHeight));
            ...
            <div className="bg-white rounded-2xl shadow-2xl overflow-hidden w-full max-w-md flex flex-col">
                ...
                <div className="p-6 flex flex-col items-center gap-4 bg-slate-100 select-none">
                    ...
                    <div className="w-full space-y-2 px-4">
                        ...
                        <p className="text-[10px] text-center text-gray-400 mt-2">Drag to move • Drag corners to resize • Zoom slider<br/>(Full original image preserved for re-cropping)</p>
                    </div>
                </div>

                <div className="p-4 bg-white border-t border-gray-100 flex flex-col gap-3">
                    {/* Skip crop — only when it's a brand new upload */}
                    {isNewUpload && (
                        <button
                            onClick={() => onCropComplete(imageSrc)}
                            className="w-full flex items-center justify-center gap-2 bg-amber-50 hover:bg-amber-100 border border-amber-300 text-amber-800 px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-sm"
                        >
                            <UploadCloud size={15} /> Use Original — Skip Crop (Submit for Approval)
                        </button>
                    )}
                    {onNewPhoto && (
                        <label className="w-full flex items-center justify-center gap-2 cursor-pointer bg-slate-50 hover:bg-brand-50 border border-dashed border-slate-300 hover:border-brand-400 text-slate-500 hover:text-brand-600 px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest transition-all">
                            <UploadCloud size={15} /> Upload New Photo
                            <input type="file" accept="image/*" className="hidden" onChange={(e) => { onNewPhoto(e); }} />
                        </label>
                    )}
                    <div className="flex gap-3">
                        ...
                        <Button onClick={handleCrop} variant="primary" className="flex-1">
                            <Check size={18} className="mr-2" /> Save Cropped
                        </Button>
                    </div>
                </div>
            </div>
