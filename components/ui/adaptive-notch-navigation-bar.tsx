...
export function NotchNav({
  ...
  return (
    <div
      className={cn(
        "fixed inset-0 h-[100dvh] w-screen overflow-hidden bg-slate-950 p-0 md:p-2 transition-colors duration-200 dark:bg-slate-900",
        className
      )}
      {...props}
    >
      <div className="relative flex h-full w-full flex-col rounded-none md:rounded-2xl bg-white text-slate-900 antialiased transition-colors duration-200">
        <div
          aria-hidden="true"
        ...
        {showLogo && logo && (
          <aside
            ...
            className={cn(
              "hidden xl:flex absolute left-0 z-50 h-12 px-5 select-none transition-colors duration-200 bg-slate-950 dark:bg-slate-900 dark:text-zinc-950",
              isBottom
                ? "bottom-0 rounded-tr-[24px] md:items-end"
                : "top-0 rounded-br-[24px] md:items-baseline"
            )}
          >
            <div className="flex items-center text-zinc-50 dark:text-zinc-950 pt-1">
              {logo}
            </div>

            <NotchRightWing position={position} className="text-slate-950" />

            <NotchCornerLeftWing position={position} className="text-slate-950" />
          </aside>
        )}
        ...
        <header
          ...
          className={cn(
            "hidden xl:flex absolute left-1/2 -translate-x-1/2 z-50 h-12 px-4 bg-slate-950 text-zinc-50 select-none transition-colors duration-200 dark:bg-slate-900 dark:text-zinc-950",
            isBottom
              ? "bottom-0 rounded-t-[24px] md:items-end"
              : "top-0 rounded-b-[24px] md:items-start pt-1.5"
          )}
        >
          <NotchLeftWing position={position} className="text-slate-950" />
        ...
        {showRightContent && rightContent && (
          <aside
            ...
            className={cn(
              "hidden xl:flex absolute right-0 z-50 h-12 px-5 select-none transition-colors duration-200 bg-slate-950 dark:bg-slate-900 dark:text-zinc-950",
              isBottom
          ...
          >
            ...
            <NotchCornerRightWing position={position} className="text-slate-950" />
        ...
        <div
          ref={containerRef}
          className={cn(
            "xl:hidden absolute z-50 flex flex-col bg-slate-950 text-zinc-50 select-none transition-colors duration-200 dark:bg-slate-900 dark:text-zinc-950",
        ...
        >
          ...
          <div
            className={cn(
              "w-auto xl:w-max lg:w-full flex h-12 sm:h-12 items-center justify-between gap-3 sm:gap-5",
              isBottom ? "sm:items-baseline md:items-end" : "sm:items-baseline md:items-center"
            )}
          >
            {/* Left Logo Slot */}
            {showLogo && logo && (
            ...
            <button
              ...
              className="group flex h-9 sm:h-9 w-full cursor-pointer items-center justify-center gap-1.5 rounded-full px-3 py-2 text-xs sm:text-sm font-semibold text-zinc-50 outline-none transition-colors sm:hover:bg-zinc-800 focus-visible:ring-2 focus-visible:ring-zinc-400 dark:text-zinc-950 dark:sm:hover:bg-zinc-300/60 dark:focus-visible:ring-zinc-500"
            >
              {activeItem?.icon && (
        ...
        <div
          className={cn(
            "relative flex w-full items-start h-full overflow-y-auto overflow-x-hidden",
            isBottom ? "pb-16" : "pt-16"
          )}
        >
          {children}
        </div>
