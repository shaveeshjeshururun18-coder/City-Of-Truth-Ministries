...
export const api = {
    ...
    completeReboot: async (password: string): Promise<{ success: boolean; message: string; details?: any }> => {
        ...
        if (!REBOOT_PASSWORD) {
            console.error('CRITICAL: Reboot password is not configured in environment variables.');
            return { success: false, message: 'System reboot is disabled due to missing configuration.' };
        }
